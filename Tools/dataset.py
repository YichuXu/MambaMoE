...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    if (dataset_name == 'sa'):
        ...
        rgb_bands = [0, 1, 2]  # to be edited
        undefined_label_index = 0
    elif (dataset_name == 'SZUTreeHSI-R1'):
        image = io.loadmat(os.path.join(dataset_dir, dataset_name, "tree1.mat"))
        image = image['data']
        gt = io.loadmat(os.path.join(dataset_dir, dataset_name, "tree1_gt.mat"))
        gt = gt['gt']
        labels = [
            "Undefined",
            "c1",
